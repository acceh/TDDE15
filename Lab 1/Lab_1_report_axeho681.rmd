...
As one can see in the print statement above, as well as in the plots, the hill-climbing algorithm  can return non-equivalent BN-structures. The reason for this is mainly the score evaluation method as that changes how the hill-climbing method is made. In addtion, the restarts indicates how many times the algorithm restarts, which also contributes to how the BN ends up at the end of the algorithm.
...
The reason for the confusion matrices being the same in task 2 and task 3 is because the dependence of "S" is the same in both cases, which is "L" and "B". 

The confusion matrix is different in task 4 as that has a completely different dependency where all the nodes are dependent on "S" instead. This even though they may not have any dependence at all in the real world. This leads to an overfitting of the data compared to the previous cases where in depended on only "L" and "B".
